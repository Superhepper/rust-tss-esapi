...
use crate::{
    ffi::{data_zeroize::FfiDataZeroize, take_from_esys},
...
///
/// # Details
/// Makes sure that no data is left on the heap when
/// the object gets dropped.
pub(crate) struct CreateCommandOutputHandler {
    // They are stored as Option to be able to indicate
    // to the drop function if there is any data on the
    // heap that it needs to handle.
...
impl TryFrom<CreateCommandOutputHandler> for CreateKeyResult {
    ...
    fn try_from(mut ffi_data_handler: CreateCommandOutputHandler) -> Result<CreateKeyResult> {
        // The way things are done is very intentional. All data needs to be converted to owned
        // so that the data allocated on the heap gets zeroized and freed. It is then as soon
        // as possible converted to native types so that the sensitive data is stored in structures
        // that can zeroize it. After these steps are taken it is relatively safe to check for
        // errors and return the result.
        let out_private_result = ffi_data_handler
            .ffi_out_private_ptr
            .take()
            .ok_or(Error::local_error(crate::WrapperErrorKind::InternalError)) // Should never happen
...
impl Drop for CreateCommandOutputHandler {
    fn drop(&mut self) {
        ...
        // and that the owned data gets zeroized.
        if let Some(ptr) = self.ffi_out_private_ptr.take() {
            let _ = unsafe { take_from_esys(ptr) }.map(|mut v| v.ffi_data_zeroize());
        }

        if let Some(ptr) = self.ffi_out_public_ptr.take() {
            let _ = unsafe { take_from_esys(ptr) }.map(|mut v| v.ffi_data_zeroize());
        }

        if let Some(ptr) = self.ffi_creation_data_ptr.take() {
            let _ = unsafe { take_from_esys(ptr) }.map(|mut v| v.ffi_data_zeroize());
        }

        if let Some(ptr) = self.ffi_creation_hash_ptr.take() {
            let _ = unsafe { take_from_esys(ptr) }.map(|mut v| v.ffi_data_zeroize());
        }

        if let Some(ptr) = self.ffi_creation_ticket_ptr.take() {
            let _ = unsafe { take_from_esys(ptr) }.map(|mut v| v.ffi_data_zeroize());
        }
    }
}
